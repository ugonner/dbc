...
export async function joinRoom(socket: Socket, dto: JoinRoomDTO) {
  ...
  export async function canJoinRoom(userId: string, roomId: string): Promise<ICanJoinAs | undefined> {
    try{
      ...
      if(aidServiceProvided?.aidServiceId === 2) canJoinAs.isSpecialPresenter = true

      if(canJoinAs.isOwner || canJoinAs.isInvitee || canJoinAs.isAidServiceProvider || canJoinAs.isSpecialPresenter) return canJoinAs;
      return 
    }catch(error){
