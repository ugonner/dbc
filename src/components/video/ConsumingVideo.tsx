...
export const ConsumingVideo = ({producerUser, ...props}: ICallVideoProps) => {
  ...
  const userReactions = producerUser ? Object.keys(userReactionsEmojis).filter((reaction) => (producerUser as unknown as {[key: string]: unknown})[reaction] ) : null;
    ...
    <div>
      {producerUser?.isVideoTurnedOff && (
        <div>
          <IonItem>
            <IonThumbnail>
              <img src={producerUser?.avatar ? producerUser.avatar : "https://beentos.com/logo.png"} alt={`${producerUser?.userName} image`} />
            </IonThumbnail>
            <IonLabel>
      ...
      <video {...videoProps} width={"100%"} height={"auto"} ref={videoRef} autoPlay playsInline hidden={producerUser?.isVideoTurnedOff }></video>
      <div>
        {
