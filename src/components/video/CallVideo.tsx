...
export const CallVideo = (props: ICallVideoProps) => {
  ...
  const defaultImage = "https://beentos.com/logo/png";
    let {mediaStream, socket, room, dataProducer,  ...videoProps} = props;
    const {videoTurnedOff, userMediaStream, userReactionsState} = useRTCToolsContextStore();

    const videoRef = useRef({} as HTMLVideoElement)
    const userReactions = Object.keys(userReactionsEmojis).filter((reaction) => (userReactionsState as {[key: string]: unknown})[reaction]);
  ...
  const startTranscription = (recognition: any) => {
    ...
    recognition.onresult = (event: any) => {
      ...
      const transcript = results
        .map((result: any) => (result.isFinal ? result[0].transcript : `[Interim: ${result[0].transcript}]`))
        .join(" ");
            ...
            <div>
               {
                <IonItem>
                    <IonThumbnail>
                        <img src={user.profile?.avatar || defaultImage} alt={user.firstName} />
                    </IonThumbnail>
                    <IonLabel>
                        <h3>{user.firstName || "You"}</h3>
                        <p>Your video is turned off</p>
                    </IonLabel>
                </IonItem>
               }
            </div>
        )}
        <video style={{width: "100%", height: "auto" }} ref={videoRef} {...videoProps} autoPlay playsInline hidden={videoTurnedOff}></video>
